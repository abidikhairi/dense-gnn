...
def main(args):
    ...
    for _ in tqdm(range(10), desc='Experiments', leave=False):
        ...
        for epoch in tqdm(range(args.epochs), desc='Training', leave=False):
            ...
            train_acc = thm.accuracy(softmax(out[train_idx]).exp(), labels[train_idx])
        ...
        with th.no_grad():
            ...
            # })
            experiment_test_accuracies.append(thm.accuracy(softmax(out[test_idx]).exp(), labels[test_idx]).item())
            experiment_test_f1.append(thm.f1_score(softmax(out[test_idx]).exp(), labels[test_idx]).item())
            experiment_test_recall.append(thm.recall(softmax(out[test_idx]).exp(), labels[test_idx]).item())
